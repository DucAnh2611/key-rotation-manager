...
**Benefits:**
- ⚡ **Faster performance** - Uses OpenSSL under the hood
- 📦 **Zero dependencies** - No external runtime dependencies
- 🔒 **More secure** - Native implementation with constant-time operations
- 📉 **Smaller bundle** - Reduced by ~8% (28KB → 26KB)
...
**Parameter renamed:** `only` → `singleton`
...
crypto.decrypt(encrypted, 'correct-password'); // → 'secret'
crypto.decrypt(encrypted, 'wrong-password');   // → '' (not throw)
crypto.decrypt('corrupted-data', 'password');  // → '' (not throw)
```
...
```
Before: Base → Events → ConfigEvents → Store → KeyManager → KM
After:  Base → Events → Store → KeyManager → KM
```
